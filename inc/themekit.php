...
class tillotson_Themekit {
	...
	/**
	 * Returns an array of sub-term objects
	 *
	 * @param 	string|array 		$tax 			The slug or array of slugs of taxonomies
	 * @param 	int 				$parent 		ID of the parent term
	 * @return 	array 								Array of sub-term objects
	 */
	public function get_subcats( $tax, $parent, $params = array(), $cache = '' ) {

		if ( empty( $tax ) ) { return 'No taxonomy provided.'; }
		if ( empty( $parent ) ) { return 'No parent term provided.'; }
		...
		$return = wp_cache_get( $cache_name, 'subterms' );
		...
		if ( false === $return ) {
			...
			if ( ! is_wp_error( $cats ) && ! empty( $cats ) ) {
				...
				wp_cache_set( $cache_name, $cats, 'subterms', 5 * MINUTE_IN_SECONDS );
