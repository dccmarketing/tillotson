...
class tillotson_Themekit {
	...
	public function more_scripts_and_styles() {
		...
		wp_enqueue_script( 'tilly-collapse', get_template_directory_uri() . '/js/collapse-submenus.min.js', array( 'jquery', 'enquire' ), '20150803', true );
	...
	public function get_carb_ID() {
		...
		foreach ( $terms as $term ) {
			...
			if ( 'carburetor' !== $term->slug ) { continue; }
	 ...
	 */
	public function get_cats( $tax, $params = array(), $cache = '' ) {
	...
	public function get_files( $type, $object ) {
	...
	/**
	 * Returns the proper logo
	 *
	 * @return [type] [description]
	 */
	public function get_home_logo( $logo ) {
		...
		switch ( $logo ) {
			...
			case 'carbs' 	: $get = 'logo'; break;
			case 'power' 	: $get = 'logo-power'; break;
			case 'racing' 	: $get = 'logo-racing'; break;

		}

		$file = $this->wp_get_attachment_by_post_name( $get );

		if ( empty( $file->ID ) ) { return; }

		$image 	= wp_prepare_attachment_for_js( $file->ID );
		$return .= '<img class="logo-' . $logo . '" src="' . $image['sizes']['full']['url'] . '" alt="' . $image['alt'] . '" />';
	...
	/**
	 * Returns an array of term objects for the requested carb type
	 *
	 * @param 		string 		$type 		Name of the carb type
	 * @return 		array 					Term objects
	 */
	public function get_product_lines( $type ) {

		if ( empty( $type ) ) { return FALSE; }

		$return = array();
		$lines 	= $this->get_cats( 'pa_product-line', array( 'hide_empty' => FALSE ), 'product-line-' . $type );

		foreach ( $lines as $line ) {

			$carbtype = get_field( 'carb_type', $line );
	...
	public function get_svg( $svg ) {
		...
		$list['facebooksquare'] 	= '<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" class="facebook"><path d="M2.89 2h14.23q.37 0 .625.255T18 2.88v14.24q0 .36-.255.62t-.625.26h-4.08v-6.2h2.08l.31-2.41h-2.39V7.85q0-.59.25-.885t.95-.295h1.28V4.51q-.66-.09-1.86-.09-1.42 0-2.265.835T10.55 7.61v1.78H8.46v2.41h2.09V18H2.89q-.37 0-.63-.26T2 17.12V2.88q0-.37.26-.625T2.89 2z"/></svg>';
	...
	public function page_body_classes( $classes ) {
		...
		if ( is_shop() || is_tax( 'pa_product-line' ) || is_tax( 'product_market' ) ) {
	...
	public function page_template_column_content( $column_name, $post_ID ) {
		...
		if ( ! empty( $name ) ) {
			...
			echo '<span class="name-template">' . esc_html( 'Default', 'tillotson-2015' ) . '</span>';
